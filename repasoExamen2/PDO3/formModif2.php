...
<body>
    <?php
        ...
        $select = $conn->prepare("select * from matriculas where dni = ?");
        $select->execute([$_POST["buscado"]]);
    ...
    <?php
        foreach ($stmt as $campo) {
            if($campo["Field"] == "foto"){
                print "<p>".$campo['Field'].": <input type='file' name='foto' value='$select[foto]'></p>";
                ...
                print "<p>".$campo['Field'].": <input type='text' name='$campo[Field]' value=". $select[$campo['Field']]."></p>";
            } 
